...
# data needed to delete a user -> UID (generated for the device) and user_id (unique for every)
    ...
    try:
        ...
        with open(csv_file, mode='r', encoding='utf-16') as file:
            ...
            for row in reader:
                ...
                print(f"Deleted user: {user_id} - {name} with password: {password} from device: {ip}")
...
#argparse section - arguments when running the script (can)
...
parser.add_argument('-d', '--devices', nargs='+', required=False, help="Choose more than one device to insret employees to")
